...
class BertEncoder():
    ...
    def __init__(self,
        ...
        self._cls_id, self._sep_id = self._tokenizer.convert_tokens_to_ids([
                                                                           '[CLS]', '[SEP]'])
    ...
    def _make_model(self):
        ...
        segment_ids = tf.keras.layers.Input(shape=(max_seq_length,),
                                            dtype=tf.int32,
                                            name="segment_ids")
        pooled, seq = self._bert_layer(
            [input_word_ids, input_mask, segment_ids])
        self._model = tf.keras.Model(
            inputs=[input_word_ids, input_mask, segment_ids], outputs=[pooled, seq])
    ...
    def _convert_id(self, s, tokenizer):
        ...
        while(len(tokens) < self._max_seq_length):
            tokens.append(0)
            input_mask.append(0)
        tokens_out = np.array(tokens).astype(np.int32)
        mask_out = np.array(input_mask).astype(np.int32)
        seq_out = np.array([0]*self._max_seq_length).astype(np.int32)
        return tokens_out, mask_out, seq_out
        ...
        for i in arrayOfSentences:
            ...
            if all_ids is None:
                ...
                all_ids = np.concatenate(
                    [all_ids, np.expand_dims(ids, 0)], axis=0)
                all_masks = np.concatenate(
                    [all_masks, np.expand_dims(masks, 0)], axis=0)
                all_segments = np.concatenate(
                    [all_segments, np.expand_dims(segments, 0)], axis=0)
    ...
    def encode(self, arrayOfSentences):
        ...
        if len(arrayOfSentences) % self.NUM_SPLIT_LENGTH > 0:
            PAD = self.NUM_SPLIT_LENGTH - \
                len(arrayOfSentences) % self.NUM_SPLIT_LENGTH
            arrayOfSentences += ["" for _ in range(PAD)]
        ids, masks, segments = self.convert_ids(arrayOfSentences)
        SIZE_SPLIT = [self.NUM_SPLIT_LENGTH for _ in range(
            (len(arrayOfSentences)+PAD)//self.NUM_SPLIT_LENGTH)]
        ...
        self.segments = tf.split(
            segments, num_or_size_splits=SIZE_SPLIT, axis=0)
        ...
        encode_out = tf.reshape(
            seq_out.stack(),
            [-1, 768]
        ) if not PAD else tf.reshape(seq_out.stack(), [-1, 768])[:-PAD]
        seq_out.close()
        return encode_out
